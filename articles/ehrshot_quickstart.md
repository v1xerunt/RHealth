# EHRShot Dataset Quick Start Guide

## Introduction

EHRShot is a benchmark for few-shot evaluation on Electronic Health
Records (EHR) data, covering multiple predictive tasks including
operational outcomes, lab values, new diagnoses, and medical imaging
findings.

This vignette demonstrates how to use the `EHRShotDataset` class and
`BenchmarkEHRShot` task to work with EHRShot data in R.
...
- **benchmark/\[task_name\]/labeled_patients.csv**: Label files for each
  prediction task
...
You can filter clinical events by specific OMOP table types to focus on
particular kinds of clinical data:
...
- **feature**: A character vector of clinical codes from the ehrshot
  events
...
1.  **Use dev mode** (`dev = TRUE`) during development to work with a
    subset of 1000 patients
2.  **Enable parallel processing** with `num_workers` parameter when
    generating samples
3.  **Use caching** by specifying `cache_dir` to avoid regenerating
    samples
...
The EHRShot dataset includes predefined train/validation/test splits. To
